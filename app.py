        ...
        if tipo_transacao == 'saque':
              ...
              if limite_diario:
                ...
                if (valor + somatotal) < 101000.00:
                  ...
                  flash('limite diario excedido','error')
...
def home():
  ...
  if 'loggedin' not in session:
    ...
    if result:
      ...
      if dataapostas:
        for row in dataapostas:
          row = list(row)  # Converte a tupla em lista, pois tuplas são imutáveis
          if isinstance(row[7], datetime):  # Verifica se o valor é datetime
              row[7] = row[7].strftime('%d/%m')  # Formata a data para 'dia/mês'
          if isinstance(row[9], datetime):  # Verifica se o valor é datetime
              row[9] = row[9].strftime('%D/%M')  # Formata a data para 'dia/mês'
              print(row)   
...
def finalizarApostas():
  ...
  if session['admin'] == 'S' and 'loggedin' in session:
    ...
    if not eventos:
      flash('Nenhum evento finalizado', 'error')
  ...
  if session['admin'] == 'N' and 'loggedin' in session:
    ...
    if request.method == 'POST':
      ...
      if data1 > data2 or data1 == data2 or data2 >= data3:
        flash('Data de inicio de apostas maior que o fim ou fim da aposta é menor ou igual ao fim do evento','error')
      ...
      #Valor mínimo não pode ser maior que o valor máximo
        ...
        flash('Valor maximo menor que o valor minimo','error')
      ...
      #Não permite criar eventos no passado
        ...
        flash('Evento não pode ser no passado','error')
  ...
  if 'loggedin' not in session or session['admin'] == 'N':
    ...
    return redirect(url_for('login'))
      ...
      if account:
        ...
        flash('Não há nenhum evento para ser aprovado','error')
...
def desaprovar_evento(evento_id):
  ...
  if session['admin'] == 'S' and 'loggedin' in session:
    ...
    #Senão
        ...
        flash(f'Erro ao enviar o email: {e}', 'error')
...
def resultEvents():
  ...
  if session['admin'] == 'N' and 'loggedin' in session:
    ...
    if result:
      flash('Eventos encontrados', 'success')
    #senao
    else:
      flash('Não há eventos relacionados', 'error')
...
def buyCredits():
  ...
  #Senão
    ...
    #exibir mensagem
    flash('admin não pode comprar créditos','error')
...
def withdrawCredits():
  ...
  if session['admin'] == 'N' and 'loggedin' in session:
    ...
    #exibir mensagem:
    flash('admin não pode sacar créditos','error')
